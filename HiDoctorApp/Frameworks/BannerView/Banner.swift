...
open class Banner: UIView {
    ...
    @objc open var minimumHeight: CGFloat = 20
    ...
    private func initializeSubviews() {
        ...
        if image == nil {
            ...
            imageView.addConstraint(imageView.constraintWithAttribute(.width, .equal, to: 20.0))
    ...
    private func adjustHeightOffset() {
        ...
        if superview === Banner.topWindow() && self.position == .top {
            ...
            let heightOffset = min(statusBarSize.height-15, statusBarSize.width) // Arbitrary, but looks nice.
            contentTopOffsetConstraint.constant = heightOffset
            minimumHeightConstraint.constant = statusBarSize.height > 0 ? minimumHeight : 20
        } else {
