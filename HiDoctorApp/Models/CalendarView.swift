...
public class CalendarView: UIView {
    ...
    internal(set) var notesdateindex = [IndexPath]()
    internal(set) var taskdateindex = [IndexPath]()
    internal(set) var bothdateindex = [IndexPath]()
    internal(set) var weekenddateindex = [IndexPath]()
    internal(set) var holidaydateindex = [IndexPath]()
...
extension CalendarView {
    ...
    public func colorDate(_ date : Date)
    {
        guard var indexPath = self.indexPathForDate(date) else { return }
        //indexPath.item = indexPath.item + 1
        self.collectionView(collectionView, didSelectItemAt: indexPath,color: true)
    }
    public func colortask(_ date : Date)
    {
        guard var indexPath = self.indexPathForDate(date) else { return }
        //indexPath.item = indexPath.item + 1
        self.collectionView(collectionView, didSelectItemAt: indexPath,task: true)
    }
    public func colorboth(_ date : Date)
    {
        guard var indexPath = self.indexPathForDate(date) else { return }
        //indexPath.item = indexPath.item + 1
        self.collectionView(collectionView, didSelectItemAt: indexPath,both: true)
    }
    public func colorHoliday(_ date : Date)
    {
        guard var indexPath = self.indexPathForDate(date) else { return }
        //indexPath.item = indexPath.item + 1
        self.collectionView(collectionView, didSelectItemAt: indexPath, holiday: true)
    }
    public func colorWeekend(_ date : Date)
    {
        guard var indexPath = self.indexPathForDate(date) else { return }
        //indexPath.item = indexPath.item + 1
        self.collectionView(collectionView, didSelectItemAt: indexPath, weekend: true)
    }
    /*
     method: - deselectDate
     params:
