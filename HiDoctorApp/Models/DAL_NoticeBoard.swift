...
class DAL_NoticeBoard: NSObject {
    ...
    func getNoticeboardAttachementDetail(msgCode:String) -> [NoticeBoardDetailModel]!
    {
        ...
        try? dbPool.read { db in
            noticeBoardList = try NoticeBoardDetailModel.fetchAll(db, "SELECT * FROM \(NOTICEBOARD_DOWNLOAD) WHERE Msg_Code = ?", arguments: [msgCode])
        }
    ...
    func getNoticeBoardDetail() -> [NoticeBoardModel]?
    {
        ...
        try? dbPool.read { db in
            noticeBoardList = try NoticeBoardModel.fetchAll(db, "SELECT * FROM \(NOTICEBOARD_DETAIL)")
        }
        return noticeBoardList
        }
    //MARK:- Delete Details
